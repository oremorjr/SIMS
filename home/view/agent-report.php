...
if(isset($_GET['date'])){
	...
	$q_s="AND  t_agent=$agent_ID  AND t_transaction_date >= '$d1' AND t_transaction_date <= '$d2' ";
		 ...
		 $q0=mysql_query("SELECT * from osd_transaction where t_paid=1 and t_mode=$mode and t_return=0 and t_void=0 $q_s group by t_transaction_date  order by t_trans_date   ");
		  ...
		  while($row=mysql_fetch_array($q1)){
			...
			<tr class="receipt-date">
				<td><?php echo get_agent_name($agent_ID);?></td>
			...
			<?php 
			$t=0;
			 $q3=mysql_query("SELECT * from osd_transaction where t_transaction_date='$r0_date' and t_paid=1 and t_void=0 and t_return=0 and  t_mode=$mode and  t_agent=$agent_ID order by TID ASC");
