...
$("#new_transaction").click(function(){
	...
	window.location='?page=edit-pos&ID='+t_cid+'&TID='+TID;

 
});

 function a1(url, windowName) { 
popupWindow1 = window.open(url, 'LUCKY VIEW '+windowName, 'height=600,width=1260,left=0,top=0,resizable=no,scrollbars=no,toolbar=no,menubar=no,location=no,directories=no,status=no');
}

function a2(url, windowName){
 popupWindow2 = window.open(url, 'LUCKY HOME '+windowName, 'height=600,width=1260,left=0,top=0,resizable=no,scrollbars=no,toolbar=no,menubar=no,location=no,directories=no,status=no');
...
$("#btn-close").click(function(){
	var ccid="<?php echo $_GET['ID'];?>";
	var ttid="<?php echo $_GET['TID'];?>";
	window.location='?page=edit-pos&ID='+ccid+'&TID='+ttid;
});
...
function add_custom_item(){
	...
	$.ajax({
		data:{tid:tid, mode:mode, empid:empid},
		url:'../include/function/pos/cart_edit_return.php',
		success:function(data){
			$("#custom-cart").html(data);
		}
	});
...
<ul class="list-group" style="margin-bottom:5px;">
  <li class="list-group-item">Customer Name : <strong id="loading-tno"><?php echo customer('c_firstname', $CID); ?></strong></li>
  <li class="list-group-item">Address : <strong  ><?php echo customer('c_address1', $CID); ?></strong></li>
 
<li class="list-group-item"><strong>Change Customer to:</strong></li>
<select name="customer"    id="customer" class=""  autocorrect="off" autocomplete="off">
 <option value="<?php echo $CID;?>"><?php echo customer_name($CID);?></option>
<?php
$customers=osd_query("osd_customer", $where="", $group="");
foreach($customers as $customer){
echo '<option value='.$customer['CID'].'>'.$customer['c_firstname'].'</option>';
}
?>
...
<!-- <a target="_blank" href="view/print-preview-home.php?mode=<?php  echo $mode; ?>&TID=<?php echo $TID;?>" class="btn btn-success">Print Lucky View Receipt</a>
<a target="_blank" href="view/print-preview.php?mode=<?php  echo $mode; ?>&TID=<?php echo $TID;?>" class="btn btn-primary">Print Lucky Home Receipt</a>
 -->
...
<div class="pos-details"></div>
 <table class=" table table-bordered table-condensed table-hover table-striped"  style="float:left;width:100%;">
 <thead>
