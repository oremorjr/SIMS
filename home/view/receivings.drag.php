...
function showItem(){
	...
	//console.log(tid);
	$("#show-btn").removeAttr("disabled", "disabled");
	$("#loading-cart").load("../include/function/pos/cart_2.php?tid="+tid + "&mode=<?php  echo $mode; ?>").fadeIn();	
}	
function refresh_tid(){
 setInterval(function() {
	$("#loading-tno").load("../include/function/pos/refresh_1.php?mode=<?php  echo $mode; ?>");
    }, 5000);
}
...
$("#show-btn").click(function(){
	...
	$("#cl").attr("href", "view/print-preview-supplier.php?mode=<?php  echo $mode; ?>&TID=" + tid );
});
function clear_tf(){
	$("#transaction").slideUp();
...
$("#compose").click(function(){		
	...
	$.ajax({
	type:"post",
	url:"../include/function/pos/compose.php?mode=<?php  echo $mode; ?>",
	data:dataString,
	success:function(data){
		refresh_tno();
...
"); ?> 
<select style="display: none;" name="pcode" data-placeholder="Choose an item..."  id="code" class="combobox input-large form-control  "   tabindex="4">
<option value="" selected="selected"> </option>
<?php
 while($row=mysql_fetch_array($query)){ ?>
<option value="<?php  echo $row['PID']; ?>" >
<div><?php  echo $row['p_pcode']; ?> - <?php  echo $row['p_name'] ?></div>
</option>	
<?php
 } ?>
</select>
...
<input type="text" name="amount-tendered" id="payment" value="" autocomplete="off" suggestions="off" autofocus="autofocus" class="positive-integer form-control">
...
document.getElementById('dragme').style.cursor = 'move';
move = true;
mousex = e.clientX; 
mousey = e.clientY;
}
