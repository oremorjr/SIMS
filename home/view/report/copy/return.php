      ...
      <div id="div-4" class="accordion-body collapse in body">
		...
		<table  id="" class="report dataTable table table-bordered table-condensed table-hover table-striped">
		 ...
		 INNER JOIN osd_unit ON (ui_uid=UID)
		  where t_paid=1 and t_mode=3 and td_mode=3 $q_s group by UIID ");
		 ...
		 INNER JOIN osd_unit_item ON (td_unit_id=UIID)	 
		 INNER JOIN osd_unit ON (ui_uid=UID)
		  where t_paid=1 and t_mode=3 and td_mode=3  and UIID='$uid' and t_paid=1 $q_s");
		  $profit=0;
		  $sale=0;
		  $show_sale=0;
		  $show_profit=0;
		  ...
		  while($row=mysql_fetch_array($q1)){
			...
			<tr>
				...
				<td><?php echo $r0_name;?>[<?php echo $uname;?>]</td>
