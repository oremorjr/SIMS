...
<div class="col-lg-12" style="min-height:470px;">
     ...
     <div class="print print-report none"><i class="glyphicon glyphicon-print"></i> Print</div>
		<table  id="" class="report dataTable table table-bordered table-condensed table-hover table-striped">
		 ...
		 $gtotal=0;
		 $gprofit=0;
		  $q0=mysql_query("SELECT * from osd_transaction where t_paid=1 and t_void=0 and t_return=0 and t_active=0 and t_mode=$mode $q_s group by t_customer_id order by TID ");
		  while($r0=mysql_fetch_array($q0)){
			$r0_date=$r0['t_transaction_date'];
			$r0_agent=$r0['t_customer_id'];
		  
		  $q1=mysql_query("SELECT * from osd_transaction where t_customer_id='$r0_agent' and t_void=0 and t_return=0 and t_active=0 and t_paid=1 and t_mode=$mode $q_s order by TID ");
		  $profit=0;
		  $sale=0;
		  $show_sale=0;
		  $show_profit=0;
		  ...
		  while($row=mysql_fetch_array($q1)){
			...
			$agent_name=customer('c_firstname', $r0_agent);
			
			}
 		
			?>
			<tr>
				<td><a href="?page=customer-report&CID=<?php echo $r0_agent;?>&d1=<?php echo $d1;?>&d2=<?php echo $d2;?>"><span   id="rank-<?php echo $r0_agent;?>"></span> <?php echo $agent_name;?></a></td>
