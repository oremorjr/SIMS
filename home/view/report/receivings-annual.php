...
<div class="col-lg-12" style="min-height:470px;">
      <div id="div-4" class="accordion-body collapse in body">
		...
		<table  id="" class="report dataTable table table-bordered table-condensed table-hover table-striped">
		 ...
		 $gtotal=0;
		 $gprofit=0;
		  $q0=mysql_query("SELECT *, YEAR(t_transaction_date) as td from osd_transaction where t_paid=1  and t_return=0 and t_void=0 and t_mode=2 and  t_active=0 $q_s $y group by YEAR(t_transaction_date) ");
		  while($r0=mysql_fetch_array($q0)){
			$r0_date=$r0['td'];
			$r0_date1=$r0['t_transaction_date'];
		  
		  $q1=mysql_query("SELECT * from osd_transaction where YEAR(t_transaction_date)='$r0_date' and t_paid=1 and t_return=0 and t_void=0 and t_mode=2 and  t_active=0 $q_s $y");
		  $profit=0;
		  $sale=0;
		  $show_sale=0;
		  $show_profit=0;
		  ...
		  while($row=mysql_fetch_array($q1)){
			...
			?>
			<tr>
				<td><a  href="?page=summary-report&type=receivings-monthly&cat=sales&year=<?php echo $year_single;?>"><?php echo $date_single;?></a></td>
