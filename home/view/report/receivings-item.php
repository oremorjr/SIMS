...
<div class="col-lg-12" >
     ...
     <div class="print print-report none"><i class="glyphicon glyphicon-print"></i> Print</div>
		<table  id="d1" class="report dataTable table table-bordered table-condensed table-hover table-striped">
		 ...
		 $gtotal=0;
		 $gprofit=0;
		  $q0=mysql_query("SELECT * from osd_transaction INNER JOIN osd_transaction_details  ON (td_TID=TID) 
		    WHERE t_active=0 and t_void=0 and t_mode=$mode and  td_mode=$mode AND td_void=0  and td_return=$tr_mode $q_s  group by td_pcode  ");
		
		  while($r0=mysql_fetch_array($q0)){
			$r0_date=$r0['td_trans_date'];
			$r0_pid=$r0['td_pcode'];
		  
		  $q1=mysql_query("SELECT * from osd_transaction INNER JOIN osd_transaction_details  ON (td_TID=TID)   where td_void=0 AND  td_mode=$mode AND td_return='$tr_mode' AND td_pcode='$r0_pid'  $q_s   ");
		  $profit=0;
		  $sale=0;
		  $show_sale=0;
		  $show_profit=0;
		 ...
		 $t_qty=0;
		  ...
		  while($row=mysql_fetch_array($q1)){
			...
			<tr>
				...
				<td class="right"><?php echo $uname;?></td> 
		 ...
		 $t_qty=0;
		  ...
		  while($row=mysql_fetch_array($q1)){
			...
			<tr>
				...
				<td><a href="?page=receivings-item-report&PID=<?php echo $pid;?>&d1=<?php echo $d1;?>&d2=<?php echo $d2;?>&return=1"><?php echo $pname;?></a></td>
