...
function showItem(){
	...
	$.ajax({
		...
		url:'../include/function/pos/cart_return.php',
...
$("#save").click(function(){
	var tno=$("#RNO").val();
	var price=$("#unit option:selected").data('price');
	console.log(price);
...
$("#show-btn").click(function(){
	...
	$("#cl").attr("href", "view/print-preview-return.php?mode=<?php  echo $mode; ?>&TID=" + tid );
});
function clear_tf(){
	$("#transaction").slideUp();
...
INNER JOIN osd_unit ON (UID=ui_uid) where ui_selling_price>0 group by ui_pid
"); ?> 
...
<div class="col-lg-4 no-right">   <input   type="button" id="compose" value="Confirm" class="btn btn-success"></div>
</div>
