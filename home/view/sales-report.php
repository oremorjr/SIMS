  ...
  <li class="active">Sales Details</li>
      ...
      <div class="print print-report none"><i class="glyphicon glyphicon-print"></i> Print</div>
		<table id="" class=" table table-bordered table-condensed table-hover table-striped receipt">
		 ...
		 $gtotal=0;
		 $gprofit=0;
		 $q0=mysql_query("SELECT * from osd_transaction where t_active=0 and  t_paid=1 and t_mode=$mode and t_return=0 and t_void=0 $q_s group by t_transaction_date  order by t_trans_date   ");
		  while($r0=mysql_fetch_array($q0)){
			$r0_date=$r0['t_transaction_date'];
			$tno=$r0['t_receiptno'];
			$tid=$r0['TID'];
		  
		  $q1=mysql_query("SELECT * from osd_transaction where  t_active=0 and  t_transaction_date='$r0_date' and t_paid=1 and t_void=0 and t_return=0 and t_mode=$mode $q_s");
		  $profit=0;
		  $sale=0;
		  $show_sale=0;
		  $show_profit=0;
		  ...
		  while($row=mysql_fetch_array($q1)){
			...
			<?php 
			$t=0;
			 $q3=mysql_query("SELECT * from osd_transaction where t_transaction_date='$r0_date' and t_paid=1 and t_void=0 and t_return=0 and  t_mode=$mode order by TID ASC");
			 ...
			 ?>
					...
					<tr>
						...
						<td width="30%"  >
 						...
 						<?php  edit_sales_receipt($t_id, $CID);?>

						<a href="#" class="show_receipt"  data-toggle="modal" data-target="#myModal"   id="<?php echo $t_id;?>"><?php echo receipt_no($t_id);?></a></td> 
