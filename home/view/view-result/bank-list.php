...
$rows=osd_query('osd_bank',$where="b_bankID!=0 ORDER BY b_bankID DESC",$group='');
$currentrows=count_sims_query('COUNT(b_bankID)','P','osd_bank')+1;
?> 
 ...
 <table class="dataTable table table-bordered table-condensed table-hover table-striped">
                      ...
                      $v1=$row['b_bankID'];
                      $v2=$row['b_bankName'];
                    $q=mysql_query("SELECT * from osd_account_ledger where al_bank=$v1 ");
                    $qc=mysql_num_rows($q);
                      ...
                      <?php endif;?>
                    <?php if(current_user('delete-bank')):?>
                     <?php if($qc==0):?>                     
