...
if($CID!=0){
$qs="$f=$CID AND YEAR(t_transaction_date)='$Y' AND MONTH(t_transaction_date)='$M' AND";
}else{
 $Y=date('Y');
  $qs="AND YEAR(t_transaction_date)='$Y' AND MONTH(t_transaction_date)='$M' ";
}
?>
...
$receipts=mysql_query("SELECT TID, t_receiptno, t_amount_t, t_trans_date, t_supplier_id, t_customer_id from osd_transaction where $qs t_mode=$MODE and t_complete=1 and t_return=0 and t_void=0 ");
