...
$receipts=mysql_query("SELECT TID, t_receiptno, t_amount_t, t_trans_date, t_supplier_id from osd_transaction INNER JOIN osd_supplier ON (SID=t_supplier_id) 

 where  t_mode=2 and t_complete=0 and t_return=0 and t_void=0 and t_active=0 and (sup_name LIKE '%$str%' OR t_receiptno LIKE '%$str%'  ) ");
...
<td width="20%" class="none">
 ...
 <?php supplier_options($TID, $customerID);?>
